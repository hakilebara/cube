...
func (m *Manager) updateTasks() {
	for _, worker := range m.Workers {
		...
		err = d.Decode(&tasks)
		if err != nil {
			log.Printf("Err marshalling tasks: %s\n", err.Error())
		}
...
func (m *Manager) SendWork() {
	if m.Pending.Len() > 0 {
		...
		if err != nil {
			log.Printf("error attempting got store task event %s: %s\n", te.ID.String(), err)
		}
...
func (m *Manager) checkTaskHealth(t task.Task) error {
	...
	log.Printf("Task %s health check respone: %v\n", t.ID, resp.StatusCode)
...
func (m *Manager) restartTask(t *task.Task) {
	...
	resp, err := http.Post(url, "application/json", bytes.NewBuffer(data))
	log.Printf("[manager] restart: POST %v\n", url)
	if err != nil {
	...
	if resp.StatusCode != http.StatusCreated {
		...
		log.Printf("Resposne error (%d): %s", e.HTTPStatusCode, e.Message)
		return
	}
...
func (m *Manager) stopTask(worker string, taskID string) {
	...
	if err != nil {
		log.Printf("error creating request to delete teask %s: %v\n", taskID, err)
		return
	}
